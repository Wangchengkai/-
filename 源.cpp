...
const int d[N + 1] = { 0,3,3,3,3,3,3,3 };
const int d1 = 3, d2 = 3, d3 = 3, d4 = 3, d5 = 3, d6 = 3, d7 = 3;
...
double a1[NN + 1] = { 0,0,500,0,500,500,0,500 };
double b1[NN + 1] = { 0,500,1000,500,1000,1500 ,500,1000 };
//均匀分布
double an[NN + 1] = { 0,0,  500, 0,  500, 500 ,0,500 };
double bn[NN + 1] = { 0,500,1000,500,1000,1500,500,1000 };
...
std::vector <std::vector<std::vector<std::vector<std::vector<std::vector<std::vector<std::vector<std::vector<std::vector<float>>>>>>>>>> f;
void vector_chushihua()
{
	...
	{
		std::vector<std::vector< std::vector  < std::vector < std::vector <std::vector<std::vector<std::vector<std::vector<float>>>>>>>>> submatrix;
		submatrix.reserve(d2 + 1);

		for (size_t j = 0; j < d2 + 1; j++)
		{
			std::vector < std::vector < std::vector   < std::vector <std::vector<std::vector<std::vector<std::vector<float>>>>>>>> submatrix1;
			...
			for (size_t j3 = 0; j3 < d3 + 1; j3++)
			{
				counti++;
				cout << counti << endl;
				std::vector < std::vector < std::vector <std::vector<std::vector<std::vector<std::vector<float>>>>>>> submatrix2;
				...
				for (size_t j4 = 0; j4 < d4 + 1; j4++)
				{
					...
					std::vector < std::vector < std::vector<std::vector<std::vector<std::vector<float>>>>>> submatrix3;
					submatrix3.reserve(d5 + 1);

					for (size_t j5 = 0; j5 < d5 + 1; j5++)
					{

						std::vector < std::vector<std::vector<std::vector<std::vector<float>>>>> submatrix4;
						submatrix4.reserve(d6 + 1);

						for (size_t j6 = 0; j6 < d6 + 1; j6++)
						{
							std::vector<std::vector<std::vector<std::vector<float>>>> submatrix5;
							submatrix5.reserve(d7 + 1);

							for (size_t j7 = 0; j7 < d7 + 1; j7++)
							{
								std::vector<std::vector<std::vector<float>>> submatrix6;
								submatrix6.reserve(N + 1);
								...
								for (size_t j11 = 0; j11 < N + 1; j11++)
								{
									...
									submatrix6.push_back(submatrix10);
								}
							...
							for (j[7] = 0; j[7] <= d7; j[7]++)
									...
									for (int n = 0; n <= N; n++)
										for (int i = 0; i <= maxcase; i++)
											...
											{
												f[j[1]][j[2]][j[3]][j[4]][j[5]][j[6]][j[7]][n][i][m] = 0;
											}




	for (int n = 1; n <= N; n++)f[0][0][0][0][0][0][0][n][0][m_b - m_s] = T0;
...
int label_youxiaoxing(int* jj, int nn)//此处输入新情形。*jj为tempj，已经经过了变换
{
	...
	int i = 0;
	//int ceshi = 900;
	while (f[jj[1]][jj[2]][jj[3]][jj[4]][jj[5]][jj[6]][jj[7]][nn][i][m_b - m_s] != 0 && i <= maxcase)//说明i处存了方案
	{
		i = i + 1;
	}//0到i-1处存了方案
	...
	else if (i >= maxcase)
	{
		...
		{
			delta[k][m] = (double)f[jj[1]][jj[2]][jj[3]][jj[4]][jj[5]][jj[6]][jj[7]][nn][k][m] - (double)f[jj[1]][jj[2]][jj[3]][jj[4]][jj[5]][jj[6]][jj[7]][nn][k][m - 1];
		}
		...
		for (int kk = 0; kk < i; kk++) {
			...
			{
				int bool11 = 0;
				for (int iii = 1; iii <= N; iii++)if (f[jj[1]][jj[2]][jj[3]][jj[4]][jj[5]][jj[6]][jj[7]][nn][kk][m_b - m_s + iii] > -bigM / 2 && new_label[m_b - m_s + iii] > -bigM / 2)bool11 = 1;
		...
		{
			if (f[jj[1]][jj[2]][jj[3]][jj[4]][jj[5]][jj[6]][jj[7]][nn][replace_num][m] < -bigM / 2 || new_label[m] < -bigM / 2)
				f[jj[1]][jj[2]][jj[3]][jj[4]][jj[5]][jj[6]][jj[7]][nn][replace_num][m] = -bigM;
			else if (f[jj[1]][jj[2]][jj[3]][jj[4]][jj[5]][jj[6]][jj[7]][nn][replace_num][m] < new_label[m])
				f[jj[1]][jj[2]][jj[3]][jj[4]][jj[5]][jj[6]][jj[7]][nn][replace_num][m] = new_label[m];
		}
		else { cout << "???" << endl; }

		//cout << "replacenum: " << replace_num << endl;
		//if (replace_num != -1)for (int m = 0; m <= m_b - m_s + N; m++)f[jj[1]][jj[2]][jj[3]][jj[4]][jj[5]][jj[6]][jj[7]][nn][replace_num][m] = new_label[m];
		boolnum = -1;
	...
	else
	{
		...
		for (int ii = 0; ii < i; ii++)
		{
			...
			for (int m = 0; m < m_b - m_s; m++)
			{
				if (f[jj[1]][jj[2]][jj[3]][jj[4]][jj[5]][jj[6]][jj[7]][nn][ii][m] < min1(new_label[m] - 0.1, new_label[m + 1] - 0.1 - processtime[nn][m + 1]) && f[jj[1]][jj[2]][jj[3]][jj[4]][jj[5]][jj[6]][jj[7]][nn][ii][m]>-bigM / 2)
				{
					//change
			...
			{
				if (f[jj[1]][jj[2]][jj[3]][jj[4]][jj[5]][jj[6]][jj[7]][nn][ii][m] < new_label[m] - 0.1 && f[jj[1]][jj[2]][jj[3]][jj[4]][jj[5]][jj[6]][jj[7]][nn][ii][m]>-bigM / 2)
				{
					boolnum11 = 0;//不被压制* *********************************************************** debug
				}
			}
		...
		if (boolnum1 == 0) //不被=制
		{
			...
			for (int ii = 0; ii < i; ii++)
			{
				...
				for (int m = 0; m < m_b - m_s; m++)
				{
					if (min1(f[jj[1]][jj[2]][jj[3]][jj[4]][jj[5]][jj[6]][jj[7]][nn][ii][m], f[jj[1]][jj[2]][jj[3]][jj[4]][jj[5]][jj[6]][jj[7]][nn][ii][m + 1] - processtime[nn][m + 1]) > new_label[m] && new_label[m] > -bigM / 2)
					{
						boolnum2 = 0;
					}
				}
				for (int m = m_b - m_s; m <= m_b - m_s + N; m++)
				{
					if ((f[jj[1]][jj[2]][jj[3]][jj[4]][jj[5]][jj[6]][jj[7]][nn][ii][m]) > new_label[m] && new_label[m] > -bigM / 2)
				...
				if (boolnum2 == 1)//压制
				{
					if (ii != i - 1)
					{
						for (int m = 0; m <= m_b - m_s + N; m++)
						{
							f[jj[1]][jj[2]][jj[3]][jj[4]][jj[5]][jj[6]][jj[7]][nn][ii][m] = f[jj[1]][jj[2]][jj[3]][jj[4]][jj[5]][jj[6]][jj[7]][nn][i - 1][m];
							f[jj[1]][jj[2]][jj[3]][jj[4]][jj[5]][jj[6]][jj[7]][nn][i - 1][m] = 0;
							//cout << "ok%%%%%%%%%%" << endl;
						}
					}
					else {
						for (int m = 0; m <= m_b - m_s + N; m++)
						{
							f[jj[1]][jj[2]][jj[3]][jj[4]][jj[5]][jj[6]][jj[7]][nn][ii][m] = 0;
						}
					}
					ii--;
					i--;
	...
	else
	{
		...
		for (int ii = 0; ii < i; ii++)
		{
			...
			for (int m = m_b - m_s + 1; m <= m_b - m_s + N; m++)
			{
				...
				{
					int bool1 = 1;
					for (int nnn = 1; nnn <= N; nnn++)if (nnn != m - m_b + m_s)if (f[jj[1]][jj[2]][jj[3]][jj[4]][jj[5]][jj[6]][jj[7]][nn][ii][m_b - m_s + nnn] - a1[nnn] <= f[jj[1]][jj[2]][jj[3]][jj[4]][jj[5]][jj[6]][jj[7]][nn][ii][m] - b1[m - m_b + m_s] && f[jj[1]][jj[2]][jj[3]][jj[4]][jj[5]][jj[6]][jj[7]][nn][ii][m] > 0 && f[jj[1]][jj[2]][jj[3]][jj[4]][jj[5]][jj[6]][jj[7]][nn][ii][m_b - m_s + nnn] > 0)bool1 = 0;//当前当前工件被认为不重要
...
void new_state_set(int* j, int n, int i, int nn, int dltj)
{
	...
	for (int m1 = 0; m1 <= m_b - m_s + N; m1++)old_label[m1] = f[j[1]][j[2]][j[3]][j[4]][j[5]][j[6]][j[7]][n][i][m1];
	//cout << "old_label:" << endl;
	...
	if (temp != -1)
	{
		...
		for (int m = 0; m <= m_b - m_s + N; m++)
		{
			f[jj[1]][jj[2]][jj[3]][jj[4]][jj[5]][jj[6]][jj[7]][nn][temp][m] = new_label[m];
		}
		//if (temp >= 100)showdouble(new_label);
		//if (temp >= 100) {
		//	for (int mm = 0; mm <= 7; mm++)
		//		cout << f[jj[1]][jj[2]][jj[3]][jj[4]][jj[5]][jj[6]][jj[7]][nn][temp][mm] << " ";
		//	cout << endl;
		//}
...
void statetransition(int* j, int n)
{
	...
	int i = 0;
	while (f[j[1]][j[2]][j[3]][j[4]][j[5]][j[6]][j[7]][n][i][m_b - m_s] != 0 && i < maxcase) { i++; }
	//0~ i-1 位置都有label
	for (int ii = 0; ii < i; ii++)
...
void statetransition_initial(int* j, int n)
{
	...
	for (int nn = 1; nn <= N; nn++)
		for (int dltj = 1; dltj <= lastnum[n]; dltj++)
		{
			...
			for (int m = 0; m <= m_b - m_s + N; m++)
			{
				f[j[1] + dltj * (n == 1)][j[2] + dltj * (n == 2)][j[3] + dltj * (n == 3)][j[4] + dltj * (n == 4)][j[5] + dltj * (n == 5)][j[6] + dltj * (n == 6)][j[7] + dltj * (n == 7)][nn][0][m] = new_label[m];
			}
...
int duqu()
{
	...
	int i = 0;
	while (f[d1][d2][d3][d4][d5][d6][d7][0][i][m_b - m_s] != 0 || f[d1][d2][d3][d4][d5][d6][d7][0][i][m_b - m_s - 1] != 0)
	{
		for (int n = 1; n <= NN; n++)LAT[i][n] = bigM / 2;
		for (int m = m_b + 1; m <= m_b + N; m++)
		{
			if (f[d1][d2][d3][d4][d5][d6][d7][0][i][m - m_s] > -bigM)
				LAT[i][m - m_b] = f[d1][d2][d3][d4][d5][d6][d7][0][i][m - m_s];
		}
		if (NN != N) {
...
double min_label_real_time_U(double* a1, double* b1)
{
	...
	for (int m = 0; m <= m_b - m_s + N; m++) {
		temp2[m] = f[d1][d2][d3][d4][d5][d6][d7][0][counti][m];
	}
...
double label_real_time(int i)//给定一个场景;给定标签编号i；
{
	...
	double case_t[N + 1] = { 0,t[1],t[2],t[3],t[4],t[5],t[6],t[7] };
