						...
						for (j[6] = 0; j[6] <= d6; j[6]++)
							for (j[7] = 0; j[7] <= d7; j[7]++)
								for (int n = 1; n <= N; n++)
								{
									...
									if (bool1 > 0)
										statetransition(j, n);
									cout << "j1:" << j[1] << " j2: " << j[2] << " j3: " << j[3] << " j4: " << j[4] << " j5: " << j[5] << "nn " << n << " MAXcase " << maxcaseused << endl;// " time0 " << f[j[1]][j[2]][j[3]][j[4]][j[5]][j[6]][j[7]][1][0][m_b - m_s] << endl;
								}
...
int label_youxiaoxing(int* jj, int nn)//此处输入新情形。*jj为tempj，已经经过了变换
{
	...
	while (f[jj[1]][jj[2]][jj[3]][jj[4]][jj[5]][jj[6]][jj[7]][nn][i][m_b - m_s] != 0 && i < maxcase)//说明i处存了方案
	{
		...
		if (i == maxcase) {
			...
			//		cout << f[jj[1]][jj[2]][jj[3]][jj[4]][jj[5]][jj[6]][jj[7]][nn][pp][mm] << " ";
			//	cout << endl;
			//}
...
int label_youxiaoxing_1(int* jj, int nn) {
	...
	{
		if (f[jj[1]][jj[2]][jj[3]][jj[4]][jj[5]][jj[6]][jj[7]][nn][k][k] < new_label[k] || f[jj[1]][jj[2]][jj[3]][jj[4]][jj[5]][jj[6]][jj[7]][nn][k][k] == 0)
		{
			for (int m = 0; m <= m_b - m_s + NN; m++)f[jj[1]][jj[2]][jj[3]][jj[4]][jj[5]][jj[6]][jj[7]][nn][k][m] = new_label[m];
			//if(new_label[3]!=0)
...
int label_youxiaoxing_2(int* jj, int nn) {
	...
	if (i >= maxcase)//启动强行替换//将斜率最接近的优势解替换掉
	{
		...
		{
			delta[k][m] = f[jj[1]][jj[2]][jj[3]][jj[4]][jj[5]][jj[6]][jj[7]][nn][k][m] - f[jj[1]][jj[2]][jj[3]][jj[4]][jj[5]][jj[6]][jj[7]][nn][k][m - 1];
		}
		...
		while (diedai_times <= control1)//搜索范围
		{
			...
			if (f[jj[1]][jj[2]][jj[3]][jj[4]][jj[5]][jj[6]][jj[7]][nn][replace_num][0] <= new_label[1])
			{
				break;
			}
			else
...
int label_youxiaoxing_3(int* jj, int nn) {
	...
	for (int ms = 0; ms < maxcase + 1; ms++)for (int m = 0; m < m_b - m_s + NN + 2; m++)tempf[ms][m] = f[jj[1]][jj[2]][jj[3]][jj[4]][jj[5]][jj[6]][jj[7]][nn][ms][m];
...
int duqu()
{
	...
	while (f[d1][d2][d3][d4][d5][d6][d7][0][i][m_b - m_s] != 0 || f[d1][d2][d3][d4][d5][d6][d7][0][i][m_b - m_s - 1] != 0)
	{
		...
		//showdouble(f[d1][d2][d3][d4][d5][d6][d7][0][i]);
